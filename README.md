...
- ⚡ **功能四** - AI agent Skill安装，调用dove进行编译烧录下载监控日志；
- 
## 下载架构
...
| 层级 | 组件 | 职责 |
|------|------|------|
| UI层   |  Extension | 用户界面交互、状态栏显示、容器视图管理 |
...
（🧩*表示未作充分验证，插件适配OS环境WIN 语言中文 如有需要可以自行按照模板适配更多平台或联系提供支持）
...
2. build*.sh git bash 脚本编译文件需要手动配置bash安装路径，详见设置；

#### 五、AI Agent Skill集成

设置里可为常用的AI Agent导入插件的编译烧录Skill和workflow，AI自动`写-编-烧-测`链路打通;
