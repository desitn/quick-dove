...
// 智能滚动常量
const SCROLL_THRESHOLD = 50; // 像素阈值，当距离底部小于此值时重新启用自动滚动

const SerialDebug = {
    isOpen: false,
    currentTheme: 'dark',    // 默认主题
    isAutomationMode: false, // 自动化模式状态
    shouldAutoScroll: true,  // 智能滚动状态
    ...
    handleMessage: function(event) {
        ...
        switch (message.command) {
            case 'updatePorts':
                ...
                break;
            case 'preSelectPort':
                // Pre-select a specific port in the dropdown (called when panel opens for a specific port)
                if (message.portPath) {
                    const portSelector = document.getElementById('portSelector');
                    if (portSelector) {
                        portSelector.value = message.portPath;
                    }
                }
                break;
            case 'updateAtConfigList':
                this.updateAtConfigList(message.configs || []);
                break;
            case 'displayAtCommands':
                this.displayAtCommands(message.commands || []);
                break;
            case 'addLog':
                this.addLog(message.content, message.level || 'info');
                break;
            case 'addReceivedData':
                this.addReceivedData(message.data);
                break;
            case 'serialConnected':
                ...
                this.addLog(`串口已连接，路径: ${message.portPath}, 波特率: ${message.baudRate}`, 'success');
                // 禁用端口选择下拉框
    ...
    setupEventListeners: function() {
        ...
        // 添加日志容器滚动事件监听器 - 实现智能滚动
        const logContainer = document.getElementById('serialLog');
        if (logContainer) {
            logContainer.addEventListener('scroll', this.handleScroll.bind(this));
        }
    ...
    toggleAutomationMode: function() {
        ...
        if (this.isAutomationMode) {
            ...
            this.addLog('添加指令->拖动排序->调整时序->快速测试', 'success');
        } else {
